...
{
  uint64_t longer = 0; // Long to be returned
  longer = bytes[LONGSIZE - 1] & 0x00000000000000FF; // Masking to isolate the bytes in the last position of the bytes arrray
  for(int b = LONGSIZE - 2; b >= 0; b--) // Iterate through the long
  {
    longer = longer << 8; // Make space for the next bytes
    longer = longer | bytes[b]; // Or the current bytes into the long
  }
  return longer;
}
...
{
  uint64_t bite = 0; // Return value
  uint64_t mask = 0xFF; // Mask
  if(byteNum >= 0 && byteNum <= 7) // Check for valid input
  {
    source = source >> byteNum*8; // Shift the source number so its bytes are on the far right
    bite = mask & source; // And the new source with the mask to only get the far right bytes
    return bite;
  }
  else {return 0;}
}
...
uint64_t Tools::getBits(uint64_t source, int32_t low, int32_t high)
{
  ...
  uint64_t lowEnd = 0; // Variable to hold the low end of the bit range
  if(low >= 0 && high <= 63 && high >= low)
  {
    lowEnd = source >> low; // Shift the source to start at the low end
...
uint64_t Tools::setBits(uint64_t source, int32_t low, int32_t high)
{
  ...
  if(low >= 0 && high <= 63 && high >= low)
  {
    ...
    uint64_t range = getBits(source,low,high);
    range = range | (mask >> (63 - (high - low)));
    range = range << low;
    source = range | source;
    return source;
  }
...
uint64_t Tools::flipBits(uint64_t source, int32_t low, int32_t high)
{
   if(low >= 0 && high <= 63 && high >= low)
  {
    uint64_t range = getBits(source,low,high); // get the specified bits
    range = ~range; // invert them
    range = range<<low; // move the select bits to the correct position
    range = clearBits(range, high+1, 63); // turn the leading 1's into 0's
    source = clearBits(source, low, high); // clear the bits at that are going to be replaced
    source = range | source; // replace the 0's with the bits desired
    return source;
...
{
   if(srclow >= 0 && dstlow >= 0 && (dstlow + length) <= 64 && (srclow + length) <= 64)
   {
      uint64_t w = getBits(source, srclow, (srclow + (length - 1) ));
      w = w << dstlow;
...
uint64_t Tools::setByte(uint64_t source, int32_t byteNum)
{
   ...
   {
    uint64_t low = (byteNum * 8);
    uint64_t high = low + 7;
    
    uint64_t w = setBits(source, low, high);
...
{
   uint8_t r = getBits(source, 63, 63);
   return r;
}
...
bool Tools::addOverflow(uint64_t op1, uint64_t op2)
{
  ...
  uint64_t ans = op1 + op2;
  uint8_t w = getBits(op1, 63, 63);
  uint8_t t = getBits(op2, 63, 63);
  uint8_t r = getBits(ans, 63, 63);
  return (w == t && w != r);
}
 ...
 *
 * @param uint64_t op that is to be treated as a 64-bit two's complement number
 * @return true if adding 1 to op would not result in an overflow; false if adding 1 to op would result in overflow
 *
 * RULES
 * 1) no loops or conditionals or switch
 * 2) no casts
